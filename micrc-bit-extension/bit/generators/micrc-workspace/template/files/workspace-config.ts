...
export async function workspaceConfig(context: WorkspaceContext) {
  ...
  configParsed['teambit.workspace/workspace'].defaultScope = context.name.split('-')[2] === 'components' || context.name.split('-')[2] === 'atoms' || context.name.split('-')[2] === 'clientends' ? 'colibri-tech.design-system' : context.defaultScope;
  // scope ui中展示的logo
